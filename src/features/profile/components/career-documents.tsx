...
export function CareerDocuments() {
  ...
  const upload = async (type: DocumentType, file?: File) => {
    ...
    try {
      ...
      showToast(`${type === 'RESUME' ? 'Resume' : 'Cover letter'} uploaded.`, 'success');
    } catch (error) {
  ...
  const remove = async () => {
    ...
    try {
      ...
      showToast('Document removed from your profile.', 'success');
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Failed to remove document', 'error');
    } finally {
      setLoadingType(null);
    }
  };
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white p-8 shadow-sm">
      <h3 className="text-lg font-semibold text-neutral-900">Career Documents</h3>
      <p className="mt-1 text-sm text-neutral-600">PDF or DOCX, up to 10 MB. Current files are attached when you apply.</p>
        ...
        {(['RESUME', 'COVER_LETTER'] as const).map((type) => {
          ...
          return (
            <div key={type} className="rounded-xl border border-neutral-200 p-4">
              <div className="flex items-start gap-3"><FileText className="text-primary-600" size={22} /><div className="min-w-0"><p className="font-semibold text-neutral-900">{label}</p><p className="truncate text-sm text-neutral-500">{document?.originalFilename || 'Not uploaded'}</p></div></div>
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                {document && <button onClick={() => setDocumentToPreview(document)} className="inline-flex min-w-0 items-center justify-center gap-2 rounded-lg border border-neutral-200 px-3 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50" aria-label={`Preview ${label}`}><Eye size={16} />Preview</button>}
                {document && <a href={`/api/documents/${document.id}/content?disposition=attachment`} className="inline-flex items-center justify-center gap-2 rounded-lg border border-neutral-200 px-3 py-2 text-sm font-medium text-neutral-600 hover:bg-neutral-50" aria-label={`Download ${label}`}><Download size={16} />Download</a>}
                {document && <button onClick={() => setDocumentToRemove(document)} className="rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50">Remove</button>}
              </div>
            </div>
          );
        })}
      </div>}
      <ConfirmationDialog isOpen={Boolean(documentToRemove)} title="Remove document?" description="It will be removed from your profile. Existing application snapshots remain available to their recruiters." confirmLabel="Remove" variant="destructive" isLoading={loadingType !== null} onClose={() => setDocumentToRemove(null)} onConfirm={() => void remove()} />
