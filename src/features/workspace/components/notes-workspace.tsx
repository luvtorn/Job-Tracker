...
type SavedTag = Omit<WorkspaceTag, '_count'> & { _count?: WorkspaceTag['_count'] };
...
export function NotesWorkspace() {
  ...
  const saveTag = async (event: FormEvent) => {
    ...
    if (!name) { setTagError(actions('tagNameRequired')); return; }
    if (name.length > 50) { setTagError(actions('tagNameTooLong')); return; }
    setSaving(true); setTagError('');
    try {
      const response = await fetch(editingTag ? `/api/tags/${editingTag.id}` : '/api/tags', { method: editingTag ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...tagForm, name }) });
      const result: { tag?: SavedTag } = await response.json();
      if (!response.ok) {
        const message = response.status === 409 ? actions('tagDuplicate') : response.status === 400 ? actions('tagInvalid') : actions('tagSaveFailed');
        throw new Error(message);
      }
      const wasEditing = Boolean(editingTag);
      if (result.tag) {
        const savedTag: WorkspaceTag = {
          ...result.tag,
          _count: result.tag._count ?? { applications: 0 },
        };
        setTags((current) => wasEditing
          ? current.map((tag) => tag.id === savedTag.id ? { ...tag, ...savedTag } : tag)
          : [...current, savedTag].sort((left, right) => left.name.localeCompare(right.name)));
      } else {
        await load();
      }
      setTagForm({ name: '', color: 'blue' }); setEditingTag(null);
      showToast(wasEditing ? actions('tagUpdated') : actions('tagCreated'), 'success');
    }
    catch (saveError) {
      const message = saveError instanceof Error ? saveError.message : actions('tagSaveFailed');
      setTagError(message); showToast(message, 'error');
    }
    finally { setSaving(false); }
  };
  ...
  return <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
    ...
    <aside className="h-fit space-y-5 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm"><div><h2 className="flex items-center gap-2 font-semibold"><TagIcon size={18} />{t('applicationTags')}</h2><p className="mt-1 text-xs text-neutral-500">{t('tagHint')}</p></div>
      <form onSubmit={saveTag} noValidate className="space-y-2"><input value={tagForm.name} maxLength={50} onChange={(event) => { setTagForm({ ...tagForm, name: event.target.value }); setTagError(''); }} placeholder={actions('tagName')} aria-invalid={Boolean(tagError)} aria-describedby="tag-error" className={`w-full rounded-lg border px-3 py-2 text-sm ${tagError ? 'border-red-400' : ''}`} /><div className="flex gap-2"><select value={tagForm.color} onChange={(event) => setTagForm({ ...tagForm, color: event.target.value as WorkspaceTag['color'] })} className="min-w-0 flex-1 rounded-lg border px-3 py-2 text-sm">{colors.map((color) => <option key={color} value={color}>{actions(`tagColor_${color}`)}</option>)}</select><button disabled={saving} className="rounded-lg bg-primary-600 px-3 text-white" aria-label={editingTag ? actions('saveTag') : actions('createTag')}>{editingTag ? <Check size={18} /> : <Plus size={18} />}</button>{editingTag && <button type="button" onClick={() => { setEditingTag(null); setTagForm({ name: '', color: 'blue' }); setTagError(''); }} className="rounded-lg border px-3" aria-label={actions('cancelTagEditing')}><X size={18} /></button>}</div>{tagError && <p id="tag-error" role="alert" className="text-xs text-red-600">{tagError}</p>}</form>
