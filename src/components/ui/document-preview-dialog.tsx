...
export function DocumentPreviewDialog({ document, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const message = loadError instanceof Error ? loadError.message : 'Failed to preview document';
        setError(message);
  ...
  return <AnimatePresence>{document && <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-950/50 p-0 backdrop-blur-sm sm:p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
    <motion.div role="dialog" aria-modal="true" aria-label={`Preview ${document.originalFilename}`} initial={{ opacity: 0, scale: 0.97, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 12 }} className="flex h-full w-full flex-col overflow-hidden bg-white shadow-2xl sm:h-[90vh] sm:max-w-5xl sm:rounded-2xl">
      <header className="flex items-center gap-3 border-b border-neutral-200 px-4 py-3 sm:px-5"><div className="min-w-0 flex-1"><h2 className="truncate font-semibold text-neutral-900">{document.originalFilename}</h2><p className="text-xs text-neutral-500">Secure document preview</p></div><a href={downloadUrl} className="inline-flex items-center gap-2 rounded-lg border border-neutral-200 px-3 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50"><Download size={16} /><span className="hidden sm:inline">Download</span></a>{objectUrl && <a href={objectUrl} target="_blank" rel="noreferrer" className="rounded-lg border border-neutral-200 p-2 text-neutral-700 hover:bg-neutral-50" aria-label="Open document in new tab"><ExternalLink size={18} /></a>}<button ref={closeRef} onClick={onClose} className="rounded-lg p-2 text-neutral-500 hover:bg-neutral-100" aria-label="Close preview"><X size={20} /></button></header>
