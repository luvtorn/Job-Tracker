...
export function LanguageSwitcher({ expanded = false }: { expanded?: boolean }) {
  ...
  return (
    <div className={expanded ? 'w-full space-y-3' : 'flex items-center gap-1'}>
      {expanded && (
      ...
      <div
        role="group"
        aria-label={t('label')}
        className={`${expanded ? 'flex w-full' : 'inline-flex'} rounded-lg border border-neutral-200 bg-white p-1 shadow-sm`}
      >
        {locales.map((item) => (
          <button
            key={item}
            ...
            className={`${expanded ? 'flex-1' : 'min-w-10'} rounded-md px-2 py-1.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-primary-300 disabled:opacity-60 ${
              locale === item
